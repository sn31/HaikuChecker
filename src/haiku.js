...
const twoConsonantsException = ["ght", "th", "sh", "ph", "th", "ch", "wh", "gh"]; //if next letter is h then do nothing
...
export class Haiku {
  ...
  splitTwoConsonants(word) {
    ...
    for (let i = 0; i < word.length - 1; i++) {
      if (twoConsonantsException.includes(word.substring(i, i+2)) && word[i + 1] == "h") {
        continue;
      }
      else if (twoConsonantsException.includes(word[i]) && !vowelsList.includes(word[i + 1])) {
        //split
      ...
      else if (!vowelsList.includes(word[i]) && !vowelsList.includes(word[i + 1])) {
        newWord = word.substring(0, i + 1) + "-" + word.substring(i + 1);
      }
      else {
